...
use crate::{
    handler::professor_quota_handler::{
        create_professor_quota, delete_professor_quota, get_professor_quota,
        get_professor_quotas, update_professor_quota,
    },
...
pub fn professor_quota_router() -> Router<Arc<AppState>> {
    Router::new()
        .route(
            "/",
            post(create_professor_quota).get(get_professor_quotas),
        )
        .route(
            "/{professor_quota_id}",

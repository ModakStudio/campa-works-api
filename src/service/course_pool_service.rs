...
pub fn create(
    ...
    CoursePoolRepository::create(conn, &new_course_pool).map_err(|_| AppError::DatabaseError)?;
...
pub fn get_all(conn: &mut PgConnection) -> Result<Vec<CoursePoolResponse>, AppError> {
    let course_pools = CoursePoolRepository::find_all(conn).map_err(|_| AppError::DatabaseError)?;
